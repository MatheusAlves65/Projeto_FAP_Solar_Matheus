...
#define ADC_CHANNEL ADC1_CHANNEL_4 // Ajuste o canal ADC conforme necessário
...
void buffer_test_task(void *pvParameter) {
    CircularBuffer1 buffer1;
    CircularBuffer2 buffer2;
    buffer_init_1(&buffer1);
    buffer_init_2(&buffer2);

    while (1) {
        // Coleta de amostras para o buffer 1
